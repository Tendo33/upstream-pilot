...
build: web
	go build -trimpath -o bin/upstream-pilot ./cmd/upstream-pilot
test: web
	go test ./...
integration:
	@test -n "$$PILOT_TEST_DATABASE_URL" || (printf 'Set PILOT_TEST_DATABASE_URL to an isolated PostgreSQL database\n' >&2; exit 1)
	go test -race ./...
run:
	go run ./cmd/upstream-pilot
demo-upstream:
