...
type Task = {kind: string; resource_type: string; started_at: string; finished_at: string | null; last_success_at: string | null; duration_ms: number | null; last_error: string; running: boolean; running_seconds: number; stale: boolean};
...
export function OperationsPage() {
 ...
 return <div className="page">
  ...
  {!data && !error ? <PageLoader/> : data && <>
   ...
   <section className="quality-editor"><h2>最近任务</h2>{data.tasks.length === 0 ? <p>暂无任务记录。</p> : data.tasks.map((task, i) => <article className="quality-notification" key={`${task.resource_type}:${task.started_at}:${i}`}><div><Badge tone={task.last_error ? "danger" : task.stale ? "danger" : task.running ? "warning" : "success"}>{task.last_error ? "失败" : task.stale ? "超时风险" : task.running ? "运行中" : "完成"}</Badge><strong>{task.kind}</strong><span>{task.resource_type}</span><span>{formatDate(task.started_at)}</span></div><p>{task.last_error || (task.running ? `已运行 ${task.running_seconds}s，租约会自动续期` : `耗时 ${task.duration_ms ?? "—"} ms`)}</p><small className="quality-incident-time">该资源上次成功 {formatDate(task.last_success_at ?? undefined)}</small></article>)}</section>
  </>}
 </div>;
}
