...
type QualityRow = {has_managed_controls:boolean;has_pending_controls:boolean;sorting_latency_ms:number|null;latency_source:"traffic"|"probe"|"unknown";reference_rate:number|null;eligible:boolean;account:Account;policy:Policy;state:QualityState;first_content_p95_ms:number|null;sample_count:number;success_percent:number|null;balance:number|null;balance_unit:string;balance_at:string|null;balance_fresh:boolean;cost_rate:number|null;cost_fresh:boolean;traffic:Traffic|null;traffic_at:string|null};
 ...
 return <div className="page">
  ...
  {!data&&!error?<PageLoader/>:data?.items.length===0?<EmptyState title="还没有上游数据" description="添加 Sub2API 站点并同步账号，然后选择模型并开启定时探测。" action={<Link className="button button-primary" to="/sites">添加站点</Link>}/>:<div className="quality-list" aria-label="上游质量列表">
   {data?.items.map(row=><article className="quality-row" key={row.account.id}>
    ...
    <p className="quality-reason">{row.state.reason}</p><p className="quality-note">排序首字 {ms(row.sorting_latency_ms)} · {{traffic:"真实请求",probe:"主动探测",unknown:"暂无足量有效延迟样本"}[row.latency_source]??"暂无足量有效延迟样本"}{row.has_pending_controls?" · 存在待确认控制，可还原基准或保留当前值并停止":""}</p>
