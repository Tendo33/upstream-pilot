...
type Policy = {
 ...
 price_rise_percent: number; auto_pause: boolean; window_seconds:number; slow_consecutive:number; auto_load_factor:boolean; auto_concurrency:boolean; capacity_percent:number;
};
type QualityState = {risks:{kind:string;level:number;recovery:number;unknown:boolean}[];evaluated_at:string|null;plan_error:string;plan_strategy:string;baseline_priority:number;last_applied_priority:number|null;desired_priority:number;tier:number;recovery_streak:number;status:string;reason:string;conflict:boolean;owned_pause:boolean};
...
type QualityRow = {native:{state:string;reason:string};controller_error:string;pending_since:string|null;has_managed_controls:boolean;has_pending_controls:boolean;sorting_latency_ms:number|null;latency_source:"traffic"|"probe"|"unknown";reference_rate:number|null;eligible:boolean;account:Account;policy:Policy;state:QualityState;first_content_p95_ms:number|null;sample_count:number;success_percent:number|null;balance:number|null;balance_unit:string;balance_at:string|null;balance_fresh:boolean;cost_rate:number|null;cost_fresh:boolean;traffic:Traffic|null;traffic_at:string|null};
type GroupPolicy={strategy:"price"|"speed"|"balanced";price_weight:number;speed_weight:number;minimum_healthy:number};
type Group = {policy:GroupPolicy;model:string;success_percent:number|null;first_content_p95_ms:number|null;id:string;site_id:string;name:string;accounts:number;schedulable:number;healthy:number;degraded:number};
...
export function QualityPage(){
 ...
 const [strategyGroup,setStrategyGroup]=useState<Group|null>(null);
 ...
 async function action(row:QualityRow,kind:"probe"|"evaluate"|"rate-sync"|"release"|"adopt"|"reference"){
  setBusy(`${row.account.id}:${kind}`);
  try{
   if(kind==="reference"){await api(`/quality/${row.account.id}/reference`,{method:"POST",...json({rate:row.cost_rate})})}
   else if(kind==="probe"||kind==="rate-sync"){
   ...
   toast(kind==="reference"?"已接受当前采购倍率为涨价比较基准":kind==="probe"?"探测已完成，结果已记录":kind==="evaluate"?"已按当前模式评估":kind==="rate-sync"?"成本倍率已采集":"已停止接管","success");await load();
 ...
 return <div className="page">
  ...
  <section className="quality-groups" aria-label="分组备用情况">
   {groups.filter(g=>!site||g.site_id===site).map(g=><article key={`${g.id}:${g.model}`} className={`quality-group ${group===g.id&&groupModel===g.model?"is-selected":""}`}><button className="quality-group-select" onClick={()=>{const active=group===g.id&&groupModel===g.model;setGroup(active?"":g.id);setGroupModel(active?null:g.model);setPage(1)}}>
    <strong>{g.name} · {g.model||"默认模型"}</strong><span>{g.healthy} 个有效候选 / {g.accounts} 个上游</span><Badge tone={g.healthy>=2?"success":"warning"}>{g.healthy>=2?"有健康备用":g.healthy===1?"仅一个健康上游":"备用资格未通过"}</Badge><span>首字 P95 {ms(g.first_content_p95_ms)} · 探测成功率 {g.success_percent==null?"—":`${g.success_percent}%`}</span>
   </button><div className="quality-actions"><Badge>{{price:"价格优先",speed:"速度优先",balanced:"均衡"}[g.policy.strategy]}</Badge><Button size="sm" onClick={()=>setStrategyGroup(g)}>分组策略</Button></div></article>)}
  </section>
  {strategyGroup&&<GroupEditor key={`${strategyGroup.id}:${strategyGroup.model}`} group={strategyGroup} close={()=>setStrategyGroup(null)} saved={()=>{setStrategyGroup(null);void load()}}/>}
  ...
  {!data&&!error?<PageLoader/>:data?.items.length===0?<EmptyState title="还没有上游数据" description="添加 Sub2API 站点并同步账号，然后选择模型并开启定时探测。" action={<Link className="button button-primary" to="/sites">添加站点</Link>}/>:<div className="quality-list" aria-label="上游质量列表">
   {data?.items.map(row=><article className="quality-row" key={row.account.id}>
    ...
    <dl className="quality-metrics"><div><dt>首字 P95</dt><dd>{ms(row.first_content_p95_ms)}</dd></div><div><dt>探测成功率</dt><dd>{row.success_percent==null?"—":`${row.success_percent.toFixed(1)}%`}<small>{row.sample_count} 个有效样本</small></dd></div><div><dt>余额</dt><dd>{row.balance==null?"—":`${row.balance.toFixed(2)} ${row.balance_unit}`}<small>{row.balance_fresh?"采集有效":"未知或已过期"}</small></dd></div><div><dt>采购倍率</dt><dd>{row.cost_rate==null?"—":row.cost_rate.toFixed(4)}<small>{row.cost_fresh?"采集有效":"未知或已过期"} · 基准 {row.reference_rate?.toFixed(4)??"—"}</small></dd></div><div><dt>当前 → 建议优先级</dt><dd>{row.account.priority} → {row.state.desired_priority}<small>人工基准 {row.state.baseline_priority}</small></dd></div></dl>
    <p className="quality-reason">{row.state.reason}</p><p className="quality-note">原生候选：{{eligible:"约束通过",blocked:"不可调度",unknown:"资格未知"}[row.native.state]??"资格未知"} · {row.native.reason}</p>{row.controller_error&&<p className="quality-error" role="status">控制失败：{row.controller_error}。请检查站点管理权限或查看历史。</p>}{row.pending_since&&<p className="quality-note">待确认控制始于 {formatDate(row.pending_since)}</p>}<p className="quality-note">排序首字 {ms(row.sorting_latency_ms)} · {{traffic:"真实请求",probe:"主动探测",unknown:"暂无足量有效延迟样本"}[row.latency_source]??"暂无足量有效延迟样本"}{row.has_pending_controls?" · 存在待确认控制，可还原基准或保留当前值并停止":""}</p>
    {!!row.state.plan_error&&<p role="status" className="quality-error">排序未执行：{row.state.plan_error}</p>}
    <div className="quality-actions">{row.state.risks?.map(r=><Badge key={r.kind} tone="warning">{{failure:"失败",slow:"首字变慢",balance:"余额",price:"涨价",legacy:"旧版待确认"}[r.kind]??r.kind} · {r.unknown?"证据未知":r.recovery?`恢复确认 ${r.recovery}`:`风险 ${r.level} 级`}</Badge>)}</div>
    <p className="quality-note">上次决策 {row.state.evaluated_at?formatDate(row.state.evaluated_at):"尚未生成"} · {row.eligible?"近期健康，可作备用":"尚不满足健康备用条件"}</p>
    <p className="quality-traffic">真实请求：{row.traffic?.status==="ok"?`${row.traffic.total-row.traffic.failed}/${row.traffic.total} 成功${row.traffic.truncated?"（最近 300 条）":""} · 首字 P95 ${ms(row.traffic.first_content_p95_ms)}`:row.traffic?.message||"尚未采集；后台每分钟独立采集"}{row.traffic_at&&` · ${formatDate(row.traffic_at)}`}</p>
    <div className="quality-actions"><Button size="sm" disabled={!!busy} onClick={()=>void action(row,"probe")} loading={busy===`${row.account.id}:probe`}><Activity size={14}/>立即探测</Button><Button size="sm" disabled={!!busy} onClick={()=>void action(row,"rate-sync")}>采集倍率</Button><Button size="sm" disabled={!!busy} onClick={()=>void action(row,"evaluate")}><ArrowDownUp size={14}/>评估策略</Button>{row.cost_fresh&&row.cost_rate!==row.reference_rate&&<Button size="sm" disabled={!!busy} onClick={()=>void action(row,"reference")}>接受当前采购基准</Button>}<Button size="sm" onClick={()=>{setSelected(row);setHistory(null)}}><Settings2 size={14}/>配置</Button><Button size="sm" disabled={!!busy} onClick={()=>void showHistory(row)}>历史</Button>{(row.has_managed_controls||row.has_pending_controls)&&<Button size="sm" disabled={!!busy} onClick={()=>void action(row,"release")}>还原基准并停止</Button>}{(row.state.conflict||row.policy.mode==="priority"||row.has_managed_controls||row.has_pending_controls)&&<Button size="sm" disabled={!!busy} onClick={()=>void action(row,"adopt")}>保留当前值并停止</Button>}</div>
   </article>)}
  </div>}
...
function PolicyEditor({row,close,saved}:{row:QualityRow;close:()=>void;saved:()=>void}){
 ...
 const numbers:{key:keyof Policy;label:string;hint?:string;min:number;max:number}[]=[{key:"window_seconds",label:"统计窗口（秒）",hint:"独立于最新证据有效期，必须不小于有效期",min:30,max:86400},{key:"slow_consecutive",label:"连续慢请求阈值",min:2,max:100},{key:"capacity_percent",label:"每级保留容量（%）",hint:"仅在独立容量开关启用时生效，最低保留 1",min:1,max:100},{key:"slow_ms",label:"首字延迟阈值（毫秒）",min:100,max:600000},{key:"failure_threshold",label:"连续失败阈值",min:2,max:100},{key:"error_percent",label:"错误率阈值（%）",min:1,max:100},{key:"minimum_samples",label:"统计最少样本",min:2,max:1000},{key:"recovery_samples",label:"每级恢复连续样本",min:2,max:100},{key:"cooldown_seconds",label:"恢复冷却（秒）",min:0,max:86400},{key:"fresh_seconds",label:"证据有效期（秒）",min:30,max:86400},{key:"priority_step",label:"无组内排序时每级降幅",min:1,max:100000},{key:"max_penalty",label:"无组内排序时最大降幅",min:1,max:1000000},{key:"price_rise_percent",label:"涨价提醒阈值（%）",min:0,max:10000}];
 ...
 return <section ref={ref} tabIndex={-1} className="quality-editor" aria-label={`${row.account.name} 的策略配置`}><div className="quality-section-heading"><h2>{row.account.name} · 策略配置</h2><Button onClick={close}>关闭</Button></div><form onSubmit={e=>void save(e)}>
  ...
  <div className="quality-checkboxes"><label><input type="checkbox" checked={enabled} onChange={e=>setEnabled(e.target.checked)}/>开启后台定时探测</label><label><input type="checkbox" checked={rate} onChange={e=>setRate(e.target.checked)}/>定时采集采购倍率</label><label><input type="checkbox" checked={policy.auto_pause} onChange={e=>setPolicy({...policy,auto_pause:e.target.checked})}/>自动模式下，确认无余额或认证失效时暂停；恢复后启用</label><label><input type="checkbox" checked={policy.auto_load_factor} onChange={e=>setPolicy({...policy,auto_load_factor:e.target.checked})}/>自动模式下，按风险级别调整负载系数</label><label><input type="checkbox" checked={policy.auto_concurrency} onChange={e=>setPolicy({...policy,auto_concurrency:e.target.checked})}/>自动模式下，按风险级别调整并发上限</label></div><p className="quality-note">跨组账号共用一个优先级；策略冲突时保留当前值并显示原因。关闭容量开关会停止该字段后续写回，还原基准请使用“还原基准并停止”。停调前检查各分组健康备用；人工暂停的账号不会被自动启用。采购倍率采集不会改变用户售价。</p>{error&&<p role="alert" className="quality-error">{error}</p>}<Button type="submit" variant="primary" loading={saving}>保存策略</Button>
 </form></section>
}
...
function GroupEditor({group,close,saved}:{group:Group;close:()=>void;saved:()=>void}){
 const [policy,setPolicy]=useState(group.policy),[busy,setBusy]=useState(false),[error,setError]=useState("");
 async function save(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");try{await api(`/quality/groups/${group.id}/policy`,{method:"PUT",...json({...policy,model:group.model})});saved()}catch(e){setError(errorMessage(e))}finally{setBusy(false)}}
 return <section className="quality-editor" aria-label="分组调度策略"><div className="quality-section-heading"><h2>{group.name} · {group.model||"默认模型"}</h2><Button onClick={close}>关闭</Button></div><form onSubmit={e=>void save(e)}><div className="quality-form-grid"><Field label="组内排序策略"><select value={policy.strategy} onChange={e=>setPolicy({...policy,strategy:e.target.value as GroupPolicy["strategy"]})}><option value="balanced">均衡：综合速度与成本</option><option value="price">价格优先：健康候选中选低价</option><option value="speed">速度优先：健康候选中选低延迟</option></select></Field>{policy.strategy==="balanced"&&<><Field label="成本权重"><Input type="number" min={0} max={100} step="any" value={policy.price_weight} onChange={e=>setPolicy({...policy,price_weight:Number(e.target.value)})}/></Field><Field label="速度权重"><Input type="number" min={0} max={100} step="any" value={policy.speed_weight} onChange={e=>setPolicy({...policy,speed_weight:Number(e.target.value)})}/></Field></>}<Field label="停调后至少保留健康上游数"><Input type="number" min={1} max={100} value={policy.minimum_healthy} onChange={e=>setPolicy({...policy,minimum_healthy:Number(e.target.value)})}/></Field></div><p className="quality-note">先按健康程度排序，再比较速度与采购成本。仅观察的账号会生成建议，自动账号才会写回。调整数字不代表所有请求立即切换，Sub2API 的会话绑定和负载仍参与选择。</p>{error&&<p role="alert" className="quality-error">{error}</p>}<Button type="submit" variant="primary" loading={busy}>保存分组策略</Button></form></section>
}
