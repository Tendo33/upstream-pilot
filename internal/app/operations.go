...
func (a *App) operationsHandler(w http.ResponseWriter, r *http.Request) error {
	...
	err := a.db.QueryRow(r.Context(), `SELECT COALESCE(jsonb_agg(v),'[]') FROM(SELECT s.id,s.name,s.connection_state,s.last_inventory_at,s.last_reconcile_at,s.usage_collection,s.traffic_collection,s.capabilities,GREATEST(0,EXTRACT(EPOCH FROM now()-s.next_inventory_at))::bigint AS inventory_lag_seconds,GREATEST(0,EXTRACT(EPOCH FROM now()-s.next_reconcile_at))::bigint AS reconcile_lag_seconds FROM sites s WHERE s.owner_id=$1 ORDER BY s.name)v`, owner.ID).Scan(&sites)
	if err != nil {
		return err
	}
	...
	var siteViews []map[string]json.RawMessage
	if err = json.Unmarshal(sites, &siteViews); err != nil {
		return err
	}
	for _, site := range siteViews {
		var capabilities upstream.Capabilities
		if err = json.Unmarshal(site["capabilities"], &capabilities); err != nil {
			return err
		}
		rows := []map[string]any{}
		counts := map[string]int{}
		for _, b := range capabilities.NativeBilling {
			state := b.Coverage(time.Now().UTC(), capabilities.CheckedAt)
			counts[state]++
			rows = append(rows, map[string]any{"account_id": b.AccountID, "name": b.Name, "status": state, "last_attempt_at": b.LastAttemptAt, "fresh_until": b.FreshUntil})
		}
		site["native_billing"], err = json.Marshal(map[string]any{"counts": counts, "accounts": rows, "checked_at": capabilities.CheckedAt})
		if err != nil {
			return err
		}
		delete(site, "capabilities")
	}
	sites, err = json.Marshal(siteViews)
	if err != nil {
		return err
	}
	pool := a.db.Stat()
