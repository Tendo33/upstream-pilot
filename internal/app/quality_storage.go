...
func (a *App) qualitySnapshot(ctx context.Context, w AccountWork, p quality.Policy) (quality.Snapshot, error) {
	...
	err = a.db.QueryRow(ctx, `SELECT observed_cost_rate,price_reference_rate,last_rate_sync_at,price_status FROM upstream_accounts WHERE id=$1`, w.ID).Scan(&snap.Rate, &snap.ReferenceRate, &snap.RateAt, &priceStatus)
	if err != nil {
		return snap, err
	}
